...
{
    BinaryNodeX<Comparable>* possibleAncestorX = find(possibleAncestor, root);

    if (find(x, possibleAncestorX) != NULL) {
        return true;
    }
    else {
        return false;
...
int BinarySearchTreeX<Comparable>::highestFull(BinaryNodeX<Comparable> *t) const
{
    ...
    if (flag == 1) {
        return max(highestFull(t->right), highestFull(t->left)) + 1;
    }
    return 0;
}
...
int BinarySearchTreeX<Comparable>::deepestFull(BinaryNodeX<Comparable> *t) const
{
    ...
    if (flag == 1) {
        return max(deepestFull(t->right), deepestFull(t->left)) + 1;
    }
    return 0;
}
